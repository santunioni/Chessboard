        ...
        for (var firstStepSize : List.of(2, 1)) {
            ...
            for (var firstStepDirection : List.of(BoardPathDirection.HORIZONTAL_LEFT, BoardPathDirection.HORIZONTAL_RIGHT)) {
                walker.walk(firstStepSize, firstStepDirection).ifPresent(h -> {
                    h.walk(lastStepSize, BoardPathDirection.VERTICAL_UP).ifPresent(u -> movements.add(new Displacement(this.board.getMyPosition(), u.getPosition())));
                    h.walk(lastStepSize, BoardPathDirection.VERTICAL_DOWN).ifPresent(d -> movements.add(new Displacement(this.board.getMyPosition(), d.getPosition())));
                });
            }
        }
