...
public record EnPassant(Color color, Position from, Position to) implements Play {
    ...
    private Piece getPawn(BoardState boardState, Color expectedColor, Position position) throws PlayValidationError {
        var piece = boardState.getPieceAt(position).orElseThrow(() -> new NoPieceAtPositionValidationError(position));
    ...
    private boolean hasVictimJumpedTwoSquaresLastRound(BoardState boardState, BoardHistory boardHistory, Position victimPosition) throws PlayValidationError {
        var victim = this.getPawn(boardState, this.color.opposite(), victimPosition);
        var lastPlay = boardHistory.getLastPlay().orElseThrow(() -> new PlayValidationError("En Passant cant be the first play."));
